  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-6">
        <Card className="p-6">
          <h2 className="text-sm font-bold text-slate-900 mb-4">시스템 설정</h2>
          <SettingsForm initial={settings} />
        </Card>

        <Card className="p-6">
          <h2 className="text-sm font-bold text-slate-900 mb-2">보안 가이드</h2>
          <ul className="text-xs text-slate-600 space-y-2 mt-3 list-disc pl-4">
      ...
      <Card>
        <div className="p-5 border-b border-slate-100">
          <h2 className="text-sm font-bold text-slate-900">관리자 감사 로그 (최근 50건)</h2>
        </div>
        <Table>
          <thead>
            <tr>
          ...
          <tbody>
            ...
            {logs.map((l) => (
              <tr key={l.id}>
                ...
                <Td className="text-xs text-slate-500 max-w-[320px] truncate">{l.detail ?? ""}</Td>
              </tr>
            ))}
          </tbody>
        </Table>
