...
export default function ContentsPageClient() {
  ...
  function updateParam(key: string, value: string) {
    ...
    router.push(`/app/contents?${params.toString()}`);
  }
      ...
      <div className="bg-white rounded-2xl border border-slate-200/80 p-4 mb-5 shadow-card">
        ...
        {showFilters && (
          <div className="flex flex-wrap gap-2 mt-3 pt-3 border-t border-slate-100">
            <Select value={statusParam} onValueChange={(v) => updateParam("status", v)}>
            ...
            <Select value={platformParam} onValueChange={(v) => updateParam("platform", v)}>
            ...
            <Select value={templateParam} onValueChange={(v) => updateParam("templateType", v)}>
      ...
      <div className="bg-white rounded-2xl border border-slate-200/80 overflow-hidden shadow-card">
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="text-xs text-slate-400 mt-1">다른 검색어나 필터를 시도해보세요</p>
            {hasActiveFilter && (
              <button onClick={clearAllFilters} className="mt-3 text-xs text-blue-600 underline">
                필터 초기화
              </button>
            )}
          </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50/60">
                <th className="text-left px-5 py-3.5 text-xs font-semibold text-slate-400 tracking-wide">콘텐츠</th>
                <th className="text-left px-4 py-3.5 text-xs font-semibold text-slate-400 hidden md:table-cell">유형</th>
                <th className="text-left px-4 py-3.5 text-xs font-semibold text-slate-400">플랫폼</th>
                <th className="text-left px-4 py-3.5 text-xs font-semibold text-slate-400">예정일</th>
                <th className="text-left px-4 py-3.5 text-xs font-semibold text-slate-400">상태</th>
                <th className="text-right px-5 py-3.5 text-xs font-semibold text-slate-400">액션</th>
              </tr>
            </thead>
            <tbody>
              {filtered.map((content, idx) => (
                ...
                >
                  <td className="px-5 py-3.5">
                    <Link href={`/app/contents/${content.id}`} className="block">
                      <p className="text-sm font-medium text-slate-800 group-hover:text-blue-700 transition-colors line-clamp-1">
                        {content.title}
                      </p>
                      <p className="text-xs text-slate-400 mt-0.5">{content.productName}</p>
                    </Link>
                  </td>
                  ...
                  <td className="px-4 py-3.5">
                    <Select
                      ...
                      onValueChange={(v) => handleStatusChange(content.id, v as ContentStatus)}
                    >
