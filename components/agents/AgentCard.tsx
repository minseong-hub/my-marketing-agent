      ...
      <div className="p-5">
        ...
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-center gap-3">
            <div className={cn("w-12 h-12 rounded-2xl flex items-center justify-center text-3xl", colors.bg)}>
              {AGENT_ICONS[agentType]}
            </div>
            <div>
              <h3 className="font-bold text-slate-800 text-lg leading-tight">{AGENT_NAMES[agentType]}</h3>
              <p className="text-sm text-slate-400 mt-0.5">{AGENT_DESCRIPTIONS[agentType]}</p>
            </div>
          </div>
        ...
        <div className={cn("flex items-center gap-2 px-3 py-2 rounded-xl mb-3", colors.bg)}>
          ...
          {relativeTime && (
            <span className="text-sm text-slate-400 ml-auto">{relativeTime}</span>
          )}
        </div>
        ...
        <div className="min-h-[48px]">
          {lastLog ? (
            <p className="text-sm text-slate-600 leading-relaxed line-clamp-3">{lastLog}</p>
          ) : currentTask ? (
            <p className="text-sm text-slate-400 italic">&quot;{currentTask}&quot;</p>
          ) : (
            <p className="text-sm text-slate-300">아직 작업 내역이 없습니다.</p>
          )}
        </div>
        ...
        {pendingApprovals > 0 && (
          <div className="mt-3 flex items-center gap-1.5 bg-amber-50 border border-amber-200 rounded-xl px-3 py-2">
            ...
            <span className="text-sm font-bold text-amber-700">승인 대기 {pendingApprovals}건</span>
          </div>
        )}
      </div>
    </motion.div>
