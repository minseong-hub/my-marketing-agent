...
const CREW_MAP: Record<AgentType, {
  ...
  marketing: {
    ...
    manualOps: [
      ...
      {
        title: "릴스 시나리오 (15초)",
        desc: "상품 → 15초 릴스 시나리오 (장면 5컷 + 자막 + 음악 분위기 + 후킹 1초)",
        placeholder: "예: 가을 무드등, 따뜻한 분위기, 침실에서 사용",
        promptTemplate: (input) => `다음 상품으로 인스타그램 릴스 15초 시나리오를 작성해 주세요. 5개 장면(각 3초)으로 나누고, 첫 1초 후킹 카피, 장면별 자막, 추천 BGM 분위기, 효과음 포인트를 포함해 주세요.\n\n상품: ${input}`,
      },
      {
        title: "부정 리뷰 답글",
        desc: "악성/부정 리뷰에 진정성 있는 답글 — 사과/원인/조치 3단 구조",
        placeholder: "예: 색상이 사진과 너무 달라요. 광고와 차이가 큽니다. ★1",
        promptTemplate: (input) => `다음 부정 리뷰에 한국 셀러 톤으로 답글을 작성해 주세요. 1) 진정성 있는 사과 2) 원인 인정 또는 양해 요청 3) 구체적 조치 또는 보상 제안 — 3단 구조로, 방어적이지 않게 80자 이내.\n\n리뷰:\n"${input}"`,
      },
      {
        title: "인플루언서 협업 DM",
        desc: "인플루언서에게 보낼 첫 협업 제안 DM — 칭찬/제안/조건 명시",
        placeholder: "예: 패션 인플루언서 5만 팔로워, 남성 캐주얼 위주",
        promptTemplate: (input) => `다음 조건의 인플루언서에게 첫 협업 제안 DM을 작성해 주세요. 1) 그 인플루언서 콘텐츠를 본 구체적 칭찬 1줄 2) 우리 브랜드/상품 한 줄 소개 3) 제안 형식(무상 협찬/페이드 협업 옵션 둘 다 제시) 4) 다음 단계 안내 — 친근하지만 정중하게, 200자 이내.\n\n인플루언서: ${input}`,
      },
    ],
    stats: [
  ...
  detail_page: {
    ...
    manualOps: [
      ...
      {
        title: "옵션 설명 카피",
        desc: "옵션 종류(색상/사이즈/소재) → 각 옵션의 차별점·추천 대상 한 줄 카피",
        placeholder: "예: 색상 5종 — 블랙/네이비/베이지/카멜/그레이",
        promptTemplate: (input) => `다음 옵션 목록 각각에 대해 '어떤 사람·상황에 추천' 한 줄(30자 이내)을 작성해 주세요. 색상은 분위기·계절감, 사이즈는 체형 가이드, 소재는 사용 환경 관점.\n\n옵션: ${input}`,
      },
      {
        title: "배송·환불 안내 카피",
        desc: "신뢰감 있는 배송/환불 안내 — 상세페이지 하단용 정형 텍스트",
        placeholder: "예: 평일 14시 이전 주문 당일 출고, 단순 변심 7일 환불, 반품비 3000원",
        promptTemplate: (input) => `다음 정책을 바탕으로 한국 소비자가 안심할 수 있는 상세페이지 배송·환불 안내 섹션을 작성해 주세요. 1) 배송 안내 2) 단순 변심 환불 조건 3) 교환·반품 절차 4) 사이즈/색상 차이 면책 — 친근하지만 명확하게, 표 형식 가능.\n\n정책: ${input}`,
      },
      {
        title: "리뷰 답글 템플릿 5종",
        desc: "★5/★4/★3/★2/★1 별점별 답글 템플릿 5종 — 즉시 복붙",
        placeholder: "예: 가을 니트, 여성 의류, 평균 별점 4.3",
        promptTemplate: (input) => `다음 상품의 리뷰 답글 템플릿 5종을 작성해 주세요. 별점별(★5/★4/★3/★2/★1) 1개씩, 각 80자 이내, 진정성 있고 셀러 톤으로. 별점이 낮을수록 사과·조치 비중을 높여 주세요.\n\n상품: ${input}`,
      },
    ],
    stats: [
  ...
  ads: {
    ...
    manualOps: [
      ...
      {
        title: "부정(제외) 키워드 추출",
        desc: "상품/카테고리 → 광고 노출에서 제외할 부정 키워드 30개 즉시 추출",
        placeholder: "예: 가을 니트 (의류) — 가격 8만원대 프리미엄 라인",
        promptTemplate: (input) => `다음 상품의 검색광고에서 효율을 떨어뜨릴 수 있는 부정(제외) 키워드 30개를 카테고리별로 분류해 작성해 주세요. 가격대·연관 없는 카테고리·중고/리뷰/리퍼·경쟁 브랜드명 등.\n\n상품: ${input}`,
      },
      {
        title: "광고 소재 5종 한 번에",
        desc: "타겟 → 메타용 광고 소재 5종(헤드라인+본문+CTA+해시태그) 한 번에",
        placeholder: "예: 가을 니트, 30대 직장 여성, 평일 출근복",
        promptTemplate: (input) => `다음 타겟에 메타(인스타그램) 광고 소재 5종을 작성해 주세요. 각 소재마다: 1) 헤드라인 30자 이내 2) 본문 100자 이내 3) CTA 1개 4) 해시태그 5개. 5종은 서로 다른 앵글(혜택/문제해결/사회적증거/긴급성/스토리)로.\n\n타겟: ${input}`,
      },
      {
        title: "비효율 광고 진단",
        desc: "ROAS·CTR 데이터 입력 → 일시정지 후보·개선 조치 진단",
        placeholder: "예: 캠페인 5개 — Meta-A ROAS 1.2x CTR 0.8%, Meta-B ROAS 4.5x CTR 2.4% ...",
        promptTemplate: (input) => `다음 캠페인 성과 데이터를 분석해 주세요. 1) 일시정지 후보 (ROAS 1.5x 미만 또는 CTR 1% 미만) 2) 우선순위 — 살릴/접을/유지 분류 3) 살릴 캠페인의 개선 조치 3가지. 한국 의류·생활용품 평균 ROAS 2.0~3.5x 기준.\n\n데이터: ${input}`,
      },
    ],
    stats: [
  ...
  finance: {
    ...
    manualOps: [
      ...
      {
        title: "환불 정책 카피",
        desc: "환불 정책 → 상세페이지/약관용 명확한 한국 셀러 톤 카피",
        placeholder: "예: 단순 변심 7일, 반품비 3000원, 미개봉 한정, 사이즈 교환 1회 무료",
        promptTemplate: (input) => `다음 환불 정책을 한국 소비자가 오해 없이 이해할 수 있도록 상세페이지·약관용 텍스트로 정리해 주세요. 1) 환불 가능 조건 2) 환불 불가 조건 3) 절차 4) 비용 부담 — 표·체크리스트 형식 권장. 친근하지만 법적 모호함 없게.\n\n정책: ${input}`,
      },
      {
        title: "단순 손익 코멘트",
        desc: "수치만 입력하면 페니가 한 문단 코멘트 + 신호등(🟢🟡🔴)",
        placeholder: "예: 매출 850만 매입 320만 광고 110만 수수료 75만 기타 60만",
        promptTemplate: (input) => `다음 한 달 재무 수치를 한 문단(80~120자)으로 평가해 주세요. 신호등 아이콘(🟢 안정 / 🟡 주의 / 🔴 위험) 1개와 함께. 영업이익률 15% 이상 🟢, 5~15% 🟡, 5% 미만 🔴 기준.\n\n수치: ${input}`,
      },
      {
        title: "부가세 신고 체크리스트",
        desc: "분기별 부가세 신고 직전 점검 — 챙겨야 할 서류·확인 항목 자동 정리",
        placeholder: "예: 1분기 신고, 일반과세자, 스마트스토어+자사몰, 광고비·매입세금계산서 보유",
        promptTemplate: (input) => `다음 조건의 부가세 신고 직전 체크리스트를 작성해 주세요. 1) 매출 자료 (각 채널별) 2) 매입 자료 (세금계산서·카드매출전표·현금영수증) 3) 광고비·물류비 등 비용 영수증 4) 신고 직전 확인할 누락 가능성 — 항목별 ✓ 체크박스 형식. ※ 정확한 신고는 세무사 확인 권장.\n\n조건: ${input}`,
      },
    ],
    stats: [
