    ...
    <div className="p-6 max-w-4xl mx-auto animate-fade-in">
      <div className="flex items-start gap-3 mb-6">
        ...
        <div className="flex-1 min-w-0">
          <h1 className="text-xl font-bold text-slate-800 leading-snug">{content.title}</h1>
          <p className="text-xs text-slate-400 mt-1">마지막 수정 · {formatDate(content.updatedAt)}</p>
        </div>
        <Select
                  ...
                  <CardContent className="p-5">
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center gap-2">
                        ...
                        <span className={cn("text-xs", charCount === 0 ? "text-slate-300" : "text-slate-500 font-medium")}>
                      ...
                      <div className="flex gap-2">
                        ...
                        >
                          {isEditing ? <><Save className="w-3 h-3 mr-1" />완료</> : <><Edit3 className="w-3 h-3 mr-1" />수정</>}
                        </Button>
                        <Button
                          variant="outline"
                          size="sm"
                          ...
                          disabled={!draft}
                        >
                          {copiedPlatform === platform ? <><Check className="w-3 h-3 mr-1 text-emerald-600" />복사됨</> : <><Copy className="w-3 h-3 mr-1" />복사</>}
                        </Button>
                      </div>
                    </div>
                    {draft ? (
                      isEditing ? (
                        ...
                        <div className="bg-slate-50 rounded-xl p-5 min-h-[280px] border border-slate-100">
                          <pre className="text-sm text-slate-700 whitespace-pre-wrap leading-relaxed font-sans">{draft}</pre>
                        </div>
                      )
                    ) : (
                      <div className="bg-slate-50 rounded-xl p-5 min-h-[200px] flex flex-col items-center justify-center border border-dashed border-slate-200">
                        <p className="text-slate-400 text-sm font-medium mb-1">초안이 아직 없습니다</p>
                        <p className="text-slate-300 text-xs text-center">수정 버튼으로 직접 작성하거나<br />생성 시 템플릿을 선택해보세요</p>
                      </div>
                    )}
                  </CardContent>
                </Card>
